...
#ifndef CPP_ATMI_TEMPLATE_FIELD_HPP
#define CPP_ATMI_TEMPLATE_FIELD_HPP
 ...
 */
  ...
  template <class T> class Tfield : public field {
    public:
       ...
       * @since v4.2.0
       */
      Tfield(): _value(0), field(BADFLDID){
        // intentional
      };
       ...
       */
      explicit Tfield ( const char *name ): _value(0), field(name) {
    ...
    protected:
      ...
      virtual int get ( buffer &b, FLDOCC32 occ ){
        ...
        if ( rc == -1 ) {
          throw buffer_exception (Ferror32, "FGET32 failed to get field %s (id: %d, occ: %d).", name().c_str(), id(), occurence() );
        }
