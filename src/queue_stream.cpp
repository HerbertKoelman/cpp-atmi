...
std::ostream& operator<<(std::ostream& out, atmi::queue_stream& qs){
  ...
  try{
    ...
    while ( rc !=  QMENOMSG ) {
      ...
      try {
        ...
        if ( (rc != QMENOMSG) && (len > 0) ) {
          ...
          if ( rc < 0 ) {
            ...
            if ( rc < 0 ) {
              throw atmi::tuxedo_exception ( tperrno, "TPEXPORT failed. Check buffer size (last buffer size was %d bytes).", s );
            }
          }
      ...
      } catch ( atmi::timeout_exception &timeout) {
        qs._queue->abort();
      }
    }
...
std::istream& operator>>(std::istream& in, atmi::queue_stream& qs){
  ...
  try {
    ...
    while (getline ( in, line)) {
      ...
      } catch ( atmi::diagnostic_exception &diagErr ) {
        qs._queue->abort ();
        throw;
      } catch (atmi::tuxedo_exception &tuxErr ) {
        qs._queue->abort ();
        throw;
      }
